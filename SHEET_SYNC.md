...
- `id`, `book`, `level`, `unit`, `passageNo`, `category`, `title`, `topic` — 지문 기본 정보.
  ...
  - `readingTime`, `words` 열은 더 이상 사용하지 않습니다. 기존 스프레드시트에 남아 있어도 사이트에서 무시되므로 그대로 두거나 지워도 됩니다.
...
- 검색창의 `키워드` 칩(태그)을 클릭하면 그 키워드로 바로 검색됩니다. 여러 키워드를 줄바꿈이나 쉼표로 붙여넣어도 그중 하나라도 일치하면 결과에 포함됩니다.
